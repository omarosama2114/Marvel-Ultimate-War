...
public class Game {
	...
	private static int attackedX;
	private static int attackedY;
	private static ArrayList<Integer> attackedXs = new ArrayList<>();
	private static ArrayList<Integer> attackedYs = new ArrayList<>();
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH) {
				...
				if (board[currx][curry] instanceof Cover) {
					...
					targets.add(target);
					cleanup(targets);
					attackedX = (int) target.getLocation().getX();
					attackedY = (int) target.getLocation().getY();
					return;
	...
	public void castAbility(Ability a)
		...
		if (a.getCastArea() == AreaOfEffect.SELFTARGET) {
			...
			attackedXs.add(getCurrentChampion().getLocation().x);
			attackedYs.add(getCurrentChampion().getLocation().y);
			...
			for (Champion c : team) {
				...
				if (distance <= a.getCastRange()) {
					targets.add(c);
					attackedXs.add(c.getLocation().x);
					attackedYs.add(c.getLocation().y);
				}
			}
		...
		for (int i = 0; i < targets.size(); i++) {
			attackedXs.add((int) targets.get(i).getLocation().getX());
			attackedYs.add((int) targets.get(i).getLocation().getY());
		}
	...
	private void cleanUp(ArrayList<Champion> targets) {
	...
	public static int getAttackedX() {
		return attackedX;
	}

	public static int getAttackedY() {
		return attackedY;
	}

	public static ArrayList<Integer> getAttackedXs() {
		return attackedXs;
	}

	public static ArrayList<Integer> getAttackedYs() {
		return attackedYs;
	}
